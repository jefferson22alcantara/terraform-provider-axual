...
// NewClientv2 creates a new Client using an already-issued bearer token.
func NewClientv2(apiUrl string, realm string, token string) (*Client, error) {
	http.DefaultTransport.(*http.Transport).TLSClientConfig = &tls.Config{InsecureSkipVerify: true}
	if token == "" {
	...
	client := oauth2.NewClient(context.Background(), staticToken)
	client.Timeout = 30 * time.Second
